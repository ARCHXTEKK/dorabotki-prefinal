...
import {View, Panel,IconButton, PanelHeader, Group, Div, List, Cell, Button, SimpleCell, AppRoot,AdaptivityProvider, 
    ConfigProvider, 
    SplitLayout, 
    SplitCol, PopoutWrapper, useAdaptivity} from '@vkontakte/vkui'; 
...
const alphabetCategories = {
    'А': ['Агентский договор', 'Арест', 'Аренда', 'Акцепт', 'Авторское право', 'Амнистия', 'Административное взыскание'],
    'Б': ['Банкротство', 'Брачный договор', 'Бездействие', 'Блокировка счетов'],
    'В': ['Вексель', 'Выгодоприобретатель', 'Взыскание ущерба'],
    'Г': ['Гражданское право', 'Гражданское собрание', 'Гражданский арбитраж', 'Гражданский арбитраж'],
    ...
    'З': ['Защита прав потребителя', 'Защита прав потребителя', 'Защита прав потребителя'],
    'И': ['Инвестиционное право', 'Инвестиционный агент', 'Инвестиционный арбитраж'],
...
const SidePanel = ({   
  ...
  const icons = [folderIcon1, folderIcon2, folderIcon3, folderIcon4, folderIcon5];
  ...
  const handleCategoryClick = async (categoryTitle) => { 
    if (isFirstCategory(categories.findIndex(category => category.name === categoryTitle))) { 
      return; 
    } 
    ...
    const response = await axios.get(`https://lawrs.ru:8000/api/court-cases/?category=${categoryTitle}&page_size=1001`);    
  ...
  const handleSubcategoryClick = (categoryTitle) => {     
    ...
    if (!categoryData[categoryTitle]) {  
      const fetchDataForCategory = async () => {    
        try {     
          const response = await axios.get(`https://lawrs.ru:8000/api/court-cases/?category=${categoryTitle}&page_size=1002`);    
          setContent(response.data.results);   
        } catch (error) {     
    ...
    <div className={`side-panel ${showSidePanel ? 'show' : 'hide'}`}> 
      ...
      <List>  
        {filteredCategories.map((category, index) => (  
          <div key={category.id}>  
            <Cell  
              className={`category-cell ${selectedCategory === category.name ? 'selected' : ''} ${isFirstCategory(index) ? 'disabled' : ''}`}
              onClick={() => handleCategoryClick(category.name)} 
            >  
              <div className="category-title">  
                <img src={criminalIcon} alt="Category Icon" className="category-icon" />  
                {category.name}  
              </div>  
            </Cell>  
            ...
            {expandedCategories[category.name] && ( 
              <List className="subcategory-list">
                {category.subcategories.map((sub, index) => (
                  <Cell
                    key={sub.id}
                    className={`subcategory-cell ${selectedCategory === sub.name ? 'selected' : ''}`}
                    onClick={() => handleSubcategoryClick(sub.name)}
                  >
                    <div className='subcategory-title'>
                      <img src={icons[index % icons.length]} alt="Subcategory Icon" className="subcategory-icon" />
                      {sub.name}
                    </div>
                  </Cell>
                ))}
  ...
  const CaseList = ({  
    ...
    const casesForPage = filteredContent.slice(startIndex, startIndex + itemsPerPage); 
            ...
            <List>   
              {casesForPage.map((caseData, index) => (   
                <Cell   
                  ...
                  style={{   
                    borderBottom:   
                      index !== casesForPage.length - 1 ? '2px solid #F5F5F5' : undefined,   
                  }}   
  ...
  const renderPageButtons = () => {
      ...
      if (totalPages > 0) {
          buttons.push(
              <button
                  key={1}
                  className={`pagination_page-button ${currentPage === 1 ? 'active' : ''}`}
      ...
      for (let i = startPage; i <= endPage; i++) {
          buttons.push(
              <button
                  key={i}
                  className={`pagination_page-button ${currentPage === i ? 'active' : ''}`}
      ...
      if (totalPages > 1) {
          buttons.push(
              <button
                  key={totalPages}
                  className={`pagination_page-button ${currentPage === totalPages ? 'active' : ''}`}
...
const RubricatorPanel = ({ selectedCategory, currentPage, totalPages, onPageChange, content, setActiveSection, setSelectedCase, filterContent }) => (
  <Group>    
    <Div>    
      <CaseList    
    ...
    <Group>
      <Div>
        ...
        <div className="welcome-block">
          ...
          <h2>Здравствуйте, <span id="user-name">Имя пользователя</span>!</h2>
          <h2>К сожалению у Вас нет доступа к данному приложению.</h2>
          </div>
          <div className='welcome-desc'>
        <span>Энциклопедия судебных актов доступна донам сообщества Мир юриспруденции.</span>
        <span>Для получения доступа вам необходимо оплатить подписку</span>
        </div>
        <div className="Welcome-buttons">
          <Button style={{marginBottom:'15px'}} onClick={() => setActiveSection('Rubricator')}>Оплатить подписку</Button>
          <Button style={{marginBottom:'15px'}}>Чем ещё полезна подписка?</Button>
          <Button style={{marginBottom:'15px'}}>Написать в службу поддержки</Button>
          <Button style={{marginBottom:'15px'}}>Перейти в сообщество</Button>
          </div>
        </div>
        </div>
      </Div>
...
const KeywordSearchPanel = ({ setActiveSection }) => { 
    ...
    const fetchDAta = async () => {
      if (!alphabetCategories['А'].length){
          ...
          await axios.post(`https://lawrs.ru:8000/api/count_cases_add/word`,
            {
              "word": true
            }
          )
            .then(response => {setAlphabetCategories(response.data)})
    ...
    const handleNextGroupClick = () => {
      setSelectedLetterGroup((prev) => Math.min(groupedAlphabet.length - 1, prev + 1));
    };
    ...
    const handlePrevLetterClick = () => {
      ...
      if (currentLetterIndex > 0) {
        const prevLetter = Object.keys(alphabetCategories)[currentLetterIndex - 1];
    ...
    const handleNextLetterClick = () => {
      ...
      if (currentLetterIndex < Object.keys(alphabetCategories).length - 1) {
        const nextLetter = Object.keys(alphabetCategories)[currentLetterIndex + 1];
    ...
    const handleSubmit = (word) => { 
      ...
      axios.post(`https://lawrs.ru:8000/api/count_cases_add/search`, {
        "document_text": word
      })
        .then(response => {setsearch_result(response.data); setActiveSection('FindPage')})
        .catch(error => console.error('Ошибка при получении данных:', error));
    };
            ...
            <Group>
              {showAllWords ? (
                <div className="word-list">
                  ...
                  <h2 className='keywordsearch-title' style={{color:"#5c5f6d", fontWeight:'400'}}> Поиск по ключевым словам (буква {selectedLetter})</h2>
                    <div className="alphabet-input-wrapper">
                      <Icon28SearchOutline className="search-icon" style={{ margin: '0px 0px 0px 2px' }} />
                      <input
                        className="alphabet-input"
                        style={{ display: 'flex', backgroundColor: '#fff', padding: '0px 5px', height: '31px', width: '100%' }}
                        type="text"
                    ...
                    <div сlassName="backtoalphabet" onClick={handleShowAlphabet} style={{ marginRight: '10px', borderRadius:'5px', color:'#fff', padding:'7px 10px 7px', display: 'flex', alignItems: 'center' }}> 
                    <img src={backtosearchicon} style={{height:'12px', padding:'2px 5px 0px 0px'}}/> 
                    Вернуться к алфавиту 
                  </div>
            <button className='prevletter' onClick={handlePrevLetterClick}style={{ marginRight: '10px',borderRadius:'5px', color:'#3871e0', border:'1px solid #3871e0'}}>&lt; Предыдущая буква</button>
            <button className='nextletter' onClick={handleNextLetterClick}style={{ marginRight: '10px',borderRadius:'5px', color:'#3871e0', border:'1px solid #3871e0'}}>Следующая буква &gt;</button>
            </div></div>
                  <List>
        ...
        <>
          ...
          <h2 className='keywordsearch-title' style={{color:"#5c5f6d", fontWeight:'400'}}> Поиск по ключевому слову</h2>
          </div>
            <div className="alphabet-nav-wrapper">
              <div className="Keywordsearch-wrapper">
                <Icon28SearchOutline className="search-icon" style={{ margin: '5px 0px 0px 2px' }} />
                <input
                  className="Keywordsearch-input"
                  style={{ display: 'flex', backgroundColor: '#fff', padding: '5px', height: '25px', width: '100%' }}
                  type="text"
                ...
                <div className="alphabet-groups">
                  {groupedAlphabet.map((group, index) => (
                    <Button
                      ...
                      className={selectedLetterGroup === index ? 'alphabet-group active' : 'alphabet-group'}
                    >
                      {group.join('')}
                      </Button>
                  ))}
                ...
                return ( 
                  <Div key={index} className="alphabet-category-cell">  
                      ...
                      {group.categories.slice(0, 6).map((word, idx) => ( 
                          ...
                          style={{color: '#818181', fontSize:'13px', fontWeight:'400'}}  
                          key={idx}  
                          >  
                          {word}  
                          </Cell>  
...
const handleLawClick = (lawId) => { 
  ...
  const getFilteredAndSortedLaws = () => { 
    let filteredLaws = lawsData.filter((law) => { 
      ...
      if (searchQuery && !law.title.toLowerCase().includes(searchQuery.toLowerCase())) return false; 

      return true; 
    }); 

    if (sortType === 'legalStrength') { 
      filteredLaws = filteredLaws.sort((a, b) => b.legalStrength - a.legalStrength); 
    } else if (sortType === 'popularity') { 
      filteredLaws = filteredLaws.sort((a, b) => b.popularity - a.popularity); 
    } else if (sortType === 'alphabetical') { 
      filteredLaws = filteredLaws.sort((a, b) => a.title.localeCompare(b.title)); 
    } 
  ...
  const startIndex = (currentPage - 1) * itemsPerPage; 
  const endIndex = Math.min(startIndex + itemsPerPage, filteredAndSortedLaws.length); 
  ...
  const filterSection = (section, searchQuery) => {
    ...
    return section.title.toLowerCase().includes(searchQuery.toLowerCase()) 
           || section.points.some(point => point.toLowerCase().includes(searchQuery.toLowerCase()));
  };
  ...
  return (
    <Group>
      ...
      {selectedLawId === null ? ( 
        ...
        <div className="law-search-controls">
            <div className="law-search-buttons">
                <div className="display-button" onClick={() => setShowDisplayOptions(prev => !prev)}>
                    <img src={displayicon} alt="Отображение" />
                    <button className="law-search-button">
                        Отображение
                    </button>
                    <img
                        src={downicon}
                ...
                <div className="sort-button" onClick={() => setShowSortOptions(prev => !prev)}>
                ...
                before={ 
                  law.icon === 'constitution' ? ( 
                    <img 
                      src={constitution} 
                      alt="Конституция" 
                      style={{ height: '16px', width: '16px', marginBottom: 'auto', marginTop: 'auto' }} 
                    /> 
                  ) : law.icon === 'book' ? ( 
                    <img 
                      src={bookicon} 
                      alt="book" 
                      style={{ height: '16px', marginBottom: 'auto', marginTop: 'auto' }} 
                    /> 
                  ) : ( 
                    <img 
                      src={fileicon} 
      ...
      <div className="law-detail">
        ...
        <div className="law-search-controls">
            <div className="law-search-buttons">
            <div сlassName="backtoalphabet" onClick={handleShowLaws} style={{ margin: '5px 10px 0px 0px', borderRadius:'5px', color:'#fff', padding:'0px 10px 0px', display: 'flex', alignItems: 'center', backgroundColor:'#3871e0', height:'32.5px'}}> 
                    <img src={backtosearchicon} style={{height:'15px', padding:'2px 5px 0px 0px'}}/> 
                    Вернуться к списку
                  </div>
                  <div className="display-button" onClick={() => setShowDisplayOptions(prev => !prev)}>
                    <img src={displayicon} alt="Отображение" />
  ...
  <div className="sections"> 
    {selectedLaw.sections.filter(section => filterSection(section, searchQuery)).map((section, index) => ( 
      <div key={index} className="section"> 
        ...
        {expandedSections[index] && ( 
          <ul> 
            {section.points.filter(point => filterPoint(point, searchQuery)).map((point, pointIndex) => (
...
const AdvancedSearch = ({ setActiveSection }) => { 
  ...
  useEffect(() => {
    ...
    if (!productionTypes.length){
      ...
      axios.get(`https://lawrs.ru:8000/api/categories/?page=1`)
        .then(response => {
          let res = [];
  ...
  const [selectedRole, setSelectedRole] = useState(PARTICIPANT_OPTIONS[0].value);
    ...
    const handleSubmit = () => { 
      ...
      axios.post(`https://lawrs.ru:8000/api/count_cases_add/search`, data)
        .then(response => {setsearch_result(response.data); setActiveSection('FindPage')})
        .catch(error => console.error('Ошибка при получении данных:', error));
    };
            ...
            <div className="advanced-search-input-group">
              <div className="advancedsearch-date-wrapper">
                <div className="advancedsearch-date-left">
                  ...
                  <div className="data-input-left" style={{ border: '1px solid #f0f0f0' }}>
                    <DatePicker
                      locale="ru"
      ...
      { true ? (       <Group>
        <div className="advanced-search-container">
          <div className="advanced-search-input-group">
            ...
            <input
              style={{display: 'flex',backgroundcolor: '#fff',borderradius: '5px',border: '1px solid #f0f0f0',padding: '0px',height: '25px', borderRadius:'5px'}}
              type="text"
              placeholder=""
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
            ...
            />
                ...
                <select 
                  style={{display: 'flex', backgroundColor: '#fff', padding: '0px', borderRadius:'5px', border:'none', color:'#5c5f6d',}} 
                  value={productionType} onChange={(e) => setProductionType(e.target.value)}> 
                  ...
                  {productionTypes.map((item, index) => (
                    <option key={index} value={item}>
                      {item}
                        ...
                        <img src={downicon}
            style={{height:'5px', marginTop:'auto', marginBottom:'auto'}}/>
            </div>
            </div>
            </div>
            <span className="advancedsearch-desc">Поиск будет осуществлен по точному слову или фразе</span>
            </div>
            ...
            />
                ...
                <text className="advanced-title">Уникальный идентификатор (УИД)</text> 
            <input
            style={{display: 'flex',backgroundcolor: '#fff',borderradius: '5px',border: '1px solid #f0f0f0',padding: '0px',height: '25px', borderRadius:'5px'}}
              type="text"
              placeholder=""
          ...
          <text className="advanced-title" style={{ paddingLeft: '20px' }}>
          Дата судебного акта:
        </text>
...
<div className="radio-switch"> 
  ...
  <label htmlFor="later-than">Позднее чем</label>
      ...
      {renderDatePicker()}
          ...
          {courts.map((item, index) => (
            ...
            <div className='inputdropicon' style={{ display: 'flex', alignItems: 'center' }}>
            <select
                style={{
                    backgroundColor: '#fff',
                    borderRadius: '5px',
               ...
               value={disputant} onChange={(e) => setDisputant(e.target.value)}>
                <option value=""></option>
          ...
          <div className="advanced-search-buttons">
            <div onClick={handleSubmit} className="advanced-find-button">  
              <img src={findicon} style={{height:'15px', paddingTop:'7.5px', paddingBottom:'7.5px', paddingLeft:'10px'}}/>           
              <button size="l" className="advanced-search">
              Найти
            </button>
            </div>       
            <div onClick={handleClear} className="advanced-clear-button">  
            <img src={clearicon} style={{height:'15px', paddingTop:'7.5px', paddingBottom:'7.5px', paddingLeft:'10px'}}/>
  ...
  const LawPanel = ({ setActiveSection, selectedCase }) => {
    ...
    useEffect(() => {
      if (selectedCase && selectedCase.id) {
        axios.get(`https://lawrs.ru:8000/api/court-case/${selectedCase.id}/`)
        ...
        <Div>
          <div className='LawPanel'>
            ...
            <div className="LawPanel-buttons">
              ...
              <div style={{fontSize:'14px', borderRadius:'5px', color:'#fff', padding:'0px 10px 0px', display: 'flex', alignItems: 'center', backgroundColor:'#3871e0', height:'30px'}} onClick={() => setActiveSection('Rubricator')}> 
                <img src={backtosearchicon} style={{height:'15px', padding:'2px 5px 0px 0px'}}/>  
                  Вернуться к поиску 
                </div> 
              </div> 
              <div className="LawPanel-buttons-right"> 
                <div className='LawPanel-buttons-right-one' style={{borderRadius:'5px', color:'rgb(56, 113, 224)', padding:'0px 10px 0px', display: 'flex', alignItems: 'center', backgroundColor:'#fff', height:'30px'}}> 
                <img src={aiicon} style={{height:'15px', padding:'0px 5px 0px 0px'}}/>  
                  Краткий пересказ 
                </div> 
                <div className='LawPanel-buttons-right-two' style={{borderRadius:'5px', color:'rgb(56, 113, 224)', padding:'0px 10px 0px', display: 'flex', alignItems: 'center', backgroundColor:'#fff', height:'30px'}}> 
                <img src={saveicon} style={{height:'15px', padding:'0px 5px 0px 0px'}}/>   
                  <a href={caseData.url.replace('doc/', 'doc/save/')}>Скачать документ</a> 
                </div>
              </div>
            </div>
            <div className="law-panel-content">
              ...
              <div className="law-panel-content-text">
                ...
                <h1>{caseData.case_type}<br></br> от {caseData.case_date}</h1>
                <div className="text-right">
                  <p>Дело N {caseData.case_number}</p>
                </div>
                <div className="bottom-text">
                  <div dangerouslySetInnerHTML={{__html: caseData.document_text}} ></div>
                </div>
              </div>
            </div>
          </div>
  ...
  const FindPage = ({setActiveSection, setSelectedCase}) => {  
    ...
    const handleCaseClick = (caseId) => {
      axios.get(`https://lawrs.ru:8000/api/court-case/${caseId}`)
        .then(response => {
          console.log('Детали дела:', response.data);
          setActiveSection('LawPanel');
          setSelectedCase(response.data);
        })
        .catch(error => console.error('Ошибка при получении текста дела:', error));
    };
      ...
      <Group>
        <Div className="FindPage">
          ...
          <div className='FindPage-buttons' style={{ display: 'flex'}}>
            <div className="backtosearch" onClick={() => setActiveSection('AdvancedSearch')} style={{ marginRight: '10px', borderRadius: '5px', color: '#fff', padding: '0px 10px', display: 'flex', alignItems: 'center' }}>  
                <img src={backtosearchicon} style={{ height: '15px', padding: '2px 5px 0px 0px' }} />  
                Вернуться к поиску  
            </div>
            <div className="display-button">
                    <img src={displayicon} alt="Отображение" style={{ height: '14px', marginTop: 'auto', marginBottom: 'auto', marginLeft:'5px' }}/>
                    <button className="law-search-button">
                        Отображение
                    </button>
                    <img
                        src={downicon}
                        alt="downicon"
                        style={{ height: '5px', marginTop: 'auto', marginBottom: 'auto', paddingRight:'5px'  }}
                    />
                </div>
                <Pagination
                currentPage={currentPage}
        ...
        <div className="FindPage-sort-buttons">  
          <div className="empty-div"></div> 
            <div className="sort-button-left">  
              <button style={{color:'#5c5f6d', fontSize:"16px", fontWeight:"bold"}}>  
                  Судебный акт  
              </button>  
          </div>  
          ...
          {currentItems.map((res, index) => (  
              <div key={index} className="FindPage-item-container" onClick={() => handleCaseClick(res.id)}>
              <div className="FindPage-item">
                ...
                <div className="FindPage-item-text"> 
                  ...
                  <div className="FindPage-item-text-desc">{res.document_text}</div>
                </div> 
  ...
  const App = () => { 
    ...
    const fetchDataForCategory = async (categoryTitle) => { 
        try { 
            ...
            const response = await axios.get(`https://lawrs.ru:8000/api/court-cases/?category=${categoryTitle}&page_size=1000`); 
            const data = response.data.results;
            setContent(data); 
            setCurrentPage(1);
    ...
    const filterContent = (content) => {
      if (searchContent === '') {
        ...
        return content.filter(item => 
          item.section_title.toLowerCase().includes(searchContent.toLowerCase()) || 
          item.section_description.toLowerCase().includes(searchContent.toLowerCase())
        ); 
      }
    };
        ...
        <ConfigProvider appearance="light">
            <AdaptivityProvider>
                <AppRoot mode="embedded">
                    <SplitLayout>
                        <SplitCol spaced={viewWidth > 3 ? true : undefined}>
                            <View activePanel="main">
                                ...
                                {!(activeSection === 'WelcomePage') && (
                                  ...
                                  {!(activeSection === 'LawSearch' || activeSection ==='WelcomePage' || activeSection ==='KeywordSearch' || activeSection ==='LawPanel'|| activeSection ==='FindPage') && ( 
                                    <div>
                                    <div className="header-with-pagination">
                                        <h2 className="rubricator-header">{sectionTitles[activeSection]}</h2>
                                        {(activeSection === 'Rubricator' && (selectedCategory !== null && selectedCategory !== undefined)) && ( 
                                            <Pagination 
                                                currentPage={currentPage} 
                                                totalPages={totalPages} 
                                                onPageChange={setCurrentPage} 
                                    ...
                                    {(activeSection === 'AdvancedSearch') && ( 
                                      <div className="warning">
                                        ...
                                        <img src={warningicon} style={{height:'15px'}}/></div>
                                        <div>
                                          Обратите внимание: вам необязательно заполнять все полня данной формы. Для начала поиска достаточно заполнения одного любого поля.
                                          </div>
                                        </div> )}
                                    </div>
                                            )} 
                                    ...
                                    <footer className="app-footer">
                                        <p>© 2024, <a href="https://vk.com/law365" target="_blank" rel="noopener noreferrer">Мир юриспруденции</a></p>
                                    </footer>
                                </Panel>
                            </View>
                        </SplitCol>
...
const headerNavigation = document.querySelectorAll('.left-button, .button-keyword, .button-lawsearch, .right-button');
